...
func (c *ClientExampleStruct) WithTimeout(ctx context.Context, timeout time.Duration) ClientExample {
	cc := new(ClientExampleStruct)

	cc.Client = c.Client
	cc.ctx = github_com_shrewx_ginx.SetClientTimeout(ctx, timeout)
